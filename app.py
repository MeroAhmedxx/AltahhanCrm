...
def ensure_shipment_deal_tables():
    ...
    if dialect == 'postgresql':
        ddl = [
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS deal_client_prices (
                id SERIAL PRIMARY KEY,
                client_id INTEGER DEFAULT 0,
                client_name TEXT DEFAULT '',
                product_name TEXT DEFAULT '',
...
def fetch_deal_items(conn, deal_id: int) -> list[dict]:
    rows = conn.execute(text("SELECT * FROM shipment_deal_items WHERE deal_id=:id ORDER BY sort_order ASC, id ASC"), {'id': deal_id}).fetchall()
    items = []
    for r in rows:
        d = dict(r._mapping)
...
def build_deal_items_summary(items: list[dict]) -> str:
    lines = []
    for item in items:
        title = item.get('display_name') or _text(item.get('product_name'))
        bits = []
        if _int(item.get('cartons_count')):
            bits.append(f"{_int(item.get('cartons_count'))} cartons")
        if _float(item.get('qty_ton')):
            bits.append(f"{_float(item.get('qty_ton')):.3f} ton")
        if title:
            lines.append(title + (f" ({', '.join(bits)})" if bits else ''))
    return ' ; '.join(lines)


def recalc_deal_totals(conn, deal_id: int) -> dict:
    items = fetch_deal_items(conn, deal_id)
    deal = conn.execute(text('SELECT * FROM shipment_deals WHERE id=:id'), {'id': deal_id}).fetchone()
    if not deal:
        return {'items': [], 'deal': None}
    if items:
        qty_ton = round(sum(_float(i.get('qty_ton')) for i in items), 3)
        cartons = sum(_int(i.get('cartons_count')) for i in items)
        net_ton = round(sum(_float(i.get('net_weight_ton')) for i in items), 3)
        gross_ton = round(sum(_float(i.get('gross_weight_ton')) for i in items), 3)
        items_total = round(sum(_float(i.get('total_amount')) for i in items), 2)
        freight = _float(getattr(deal, 'freight_amount', 0))
        unit_price = round(items_total / qty_ton, 2) if qty_ton else _float(getattr(deal, 'unit_price', 0))
        product_name = ' + '.join([i['display_name'] for i in items[:3]])
        if len(items) > 3:
            product_name += f" +{len(items)-3} more"
        product_description = build_deal_items_summary(items)
...
def build_deal_validation(conn, deal, shipment=None, items: list[dict] | None = None) -> list[str]:
    items = items if items is not None else fetch_deal_items(conn, deal.id)
    issues = []
    ...
    if items:
        ...
        if _float(getattr(deal, 'unit_price', 0)) <= 0:
            issues.append('Unit price must be greater than zero.')
    if _float(getattr(deal, 'gross_weight_ton', 0)) <= 0:
        issues.append('Gross weight is missing.')
    if _float(getattr(deal, 'net_weight_ton', 0)) <= 0:
        issues.append('Net weight is missing.')
    if _int(getattr(deal, 'cartons_count', 0)) <= 0:
        issues.append('Cartons count is missing.')
    if shipment is not None:
        if not _text(getattr(shipment, 'origin_port', '')):
...
def build_deal_payment_snapshot(conn, deal) -> dict:
    paid_amount = float(conn.execute(text('SELECT COALESCE(SUM(amount),0) FROM deal_payments WHERE deal_id=:id'), {'id': deal.id}).scalar() or 0)
    total_amount = float(getattr(deal, 'total_amount', 0) or 0)
    remaining_amount = round(max(total_amount - paid_amount, 0), 2)
    maturity_ts = parse_any_date_to_ts(getattr(deal, 'maturity_date', '') or '')
    now_ts = time.time()
    if total_amount <= 0:
...
def generate_deal_document_pack(conn, deal_id: int, username: str) -> dict:
    recalc = recalc_deal_totals(conn, deal_id)
    deal = recalc.get('deal') or conn.execute(text('SELECT * FROM shipment_deals WHERE id=:id'), {'id': deal_id}).fetchone()
    if not deal:
        raise HTTPException(status_code=404, detail='Deal not found')
    shipment = conn.execute(text('SELECT * FROM shipments WHERE id=:id'), {'id': deal.shipment_id}).fetchone()
    if not shipment:
        raise HTTPException(status_code=404, detail='Shipment not found')
    items = recalc.get('items') or fetch_deal_items(conn, deal_id)
    issues = build_deal_validation(conn, deal, shipment, items)
    if issues:
...
def add_customer_deal(request: Request,
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text('UPDATE shipment_deals SET status=:status WHERE id=:id'), {'status': ('Ready' if not issues else 'Draft'), 'id': deal_id})
